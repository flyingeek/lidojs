...
test('ogimetRoute', () => {
  ...
  const names = ogirte.points
                ...
                .map(p => p.name)
                .join(' ');
  expect(names).toEqual("43296 43264 43160 43109 OIKB 40851 40821 OIIK OITZ OITT 17024 15561 15499 15460 15182 LKTB 11406 10605 06484 LFPC LFPG");
});

test('ogimetData', () => {
  const data = ogimetData(ofp, wmoGrid);
  expect(data.wmo.join(' ')).toEqual("43296 43264 43160 43109 OIKB 40851 40821 OIIK OITZ OITT 17024 15561 15499 15460 15182 LKTB 11406 10605 06484 LFPC LFPG");
